...
- resolve 함수 이후 callback 을 실행하기 위한 방법으로 프로미스 resolve 함수 내부에서 callback을 실행시키는 방안을 생각해볼 수 있다.
- resolve 함수는 어떻게  callback을 참조하여 실행할 수 있을까?
...
- 그러나 이 동적인 this 를 정적으로 binding 할 수 있는 방법이 있는데, `=>  화살표 함수(arrow function)` 와 `func.bind(this)` 이다. 
- 여기서는 화살표 함수가 사용됬다. 화살표 함수 내부에서 this는 함수의 선언 시점의 this로 고정된다. 
- `() => callbakc(this.value)` 는 첫번째 프로미스의 `then` 메소드 내부에서 선언되었고, 이 메소드 내부에서 `this` 는 첫번째로 생성된 promise 인스턴스를 가르킨다. 
...
- 두번째 프로미스 내부에서 executor의 실행시에 resolve 함수에  this를  bind 시켜 인자로 전달했다.(`executor(this.resolve.bind(this), this.reject.bind(this));`)

- `resolve`는 함수이다. JS 에서 함수는 객체이다. 객체는 변수 할당시에 실제 값에 대한 참조정보를 할당한다.
- 특정 값이 함수의 인자로 전달될 때 그 값은 복사된다. 객체를 가르키는 참조값은 복사 될때 참조값을 복사하고 그 참조는 연저  동일한 객체를 가르킨다.(call by reference 인 것이다) 
...
- 함수는 객체이고 객체는 참조값으로 형태로 전달되기에 latercalls 내부에서도 resolve는 여전히 두번째 promise 의 resolve 메소드를 가르킨다.
- 결론적으로 첫번째 프로미스의 resolve 함수 내에서 latercall() -> () => (resolve(...)) 로 실행되는 resolve 는 두번째 promise의 resolve 메소드 이다..
