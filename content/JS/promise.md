...
- 두번째 프로미스가 생성될 때 constructor 내부에서 executor의 실행시에 resolve 함수에  this를  bind 시켜 인자로 전달했다.(`executor(this.resolve.bind(this), this.reject.bind(this));`) 때문에 resolve 함수 내부에서 `t his`  는 두번째 프로미스를  가르킨다.

#### latercall 이 실행될 때 해당 스코프에 resolve가 없는데 어떻게 resolve는 undefined 없이 실행될 수 있을까? 
